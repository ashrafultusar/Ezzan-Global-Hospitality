...
const services = [
  {
    ...
    description:
      "All documents are correctly prepared and verified for submission.",
  },
  {
    icon: Clock,
...
const TouristVisa = () => {
  return (
    <div className="bg-[#f8fafc]">
      {/* Hero Section */}
      <section className="py-24 bg-linear-to-r from-[#0d4a7e] via-[#1a8a81] to-[#25a18e] relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          ...
          <p className="text-xl text-white/90 max-w-2xl mx-auto font-medium">
            Hassle-free processing for Hajj, Umrah, and Global Tourism. Travel
            anywhere with expert guidance.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {destinations.map((dest, index) => (
              <div
                key={index}
                className="group relative rounded-3xl overflow-hidden shadow-md aspect-3/4 cursor-pointer bg-gray-200"
              >
                <Image
                  src={dest.image}
                  alt={`${dest.name} - Explore ${dest.country}`} // Descriptive SEO Alt
                  fill
                  loading="lazy"
                ...
                {/* Special Tag with better visibility logic */}
                {dest.isSpecial && (
                  <div className="absolute top-4 left-4 z-20 bg-amber-500 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-lg pointer-events-none">
                    Hajj & Umrah
                  </div>
                )}

                {/* Overlay - Optimized with better CSS gradient syntax if needed */}
                <div className="absolute inset-0 bg-linear-to-t from-[#0d4a7e] via-transparent to-black/20 opacity-80 group-hover:opacity-90 transition-opacity pointer-events-none" />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {additionalServices.map((service, index) => (
              ...
              >
                ...
                <h3 className="font-bold text-[#0d4a7e] text-sm">
                  {service.title}
                </h3>
              </div>
            ))}
      ...
      {/* Footer CTA Section */}
      <section className="py-24 bg-linear-to-br from-[#0d4a7e] via-[#1a8a81] to-[#25a18e]">
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <Link
              href={"/contactUs"}
              className="bg-white text-[#0d4a7e] hover:bg-slate-100 font-bold px-10 py-7 text-lg rounded-xl shadow-xl transition-all transform hover:scale-105 active:scale-95"
            >
              Free Consultation
            </Link>
          </div>
