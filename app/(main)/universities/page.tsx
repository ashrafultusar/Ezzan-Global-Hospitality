...
const UniversityCard = ({ uni }: { uni: any }) => (
  <div className="flex flex-col md:flex-row items-center justify-between p-6 bg-white border border-gray-100 rounded-lg shadow-sm mb-4 hover:shadow-md transition-shadow group">
    <div className="flex items-center gap-6 w-full">
      <div className="w-20 h-20 relative shrink-0 bg-gray-50 rounded-lg overflow-hidden border border-gray-100 transition-shadow duration-300 group-hover:shadow-sm">
        {uni?.image ? (
          <Image
            src={uni.image}
          ...
          <div
            className="w-full h-full flex items-center justify-center text-2xl font-bold text-gray-300 bg-gray-100 select-none"
            aria-hidden="true"
          >
            {uni?.name?.charAt(0) || "U"}
          </div>
